...
class Customer extends MY_Controller 
{
	...
	private function _rules($model, $request)
	{
		if (isset($model->id) && $model->id) {
			if (isset($request['contact']) && $model->contact != $request['contact']) {
				$this->form_validation->set_rules('contact', ' ', 'trim|required|numeric|is_unique[customer.contact]');
			} else {
