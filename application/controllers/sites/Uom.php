...
class Uom extends MY_Controller 
{
	function __construct()
	{
		...
		$this->_page = "Satuan";
		$this->load->model('Uom_model', 'model');
	...
	private function _rules($model, $request)
	{
		if (isset($model->id) && $model->id) {
			if (isset($request['code']) && $model->code != $request['code']) {
				$this->form_validation->set_rules('code', ' ', 'trim|required|max_length[5]|is_unique[uom.code]');
			} else {
				$this->form_validation->set_rules('code', ' ', 'trim|required|max_length[5]');
			}
		} else {
			$this->form_validation->set_rules('code', ' ', 'trim|required|max_length[5]|is_unique[uom.code]');
		}
		$this->form_validation->set_rules('description', ' ', 'trim|required|max_length[16]');
