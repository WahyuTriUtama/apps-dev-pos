<div class="box box-default">
  <div class="box-header with-border">
    <h3 class="box-title">DATA PEMBELIAN
    </h3>
  </div>
  <div class="box-body">
  	<div class="pull-left">
    ...
    <div id="dataview">
		<table id="mytable" class="table table-striped table-bordered dt-responsive nowrap display">
		  ...
		  <tbody>
		  	<?php $no=0; foreach ($model->result() as $row) { ?>
		  		<tr>
		  			<td><?= ++$no; ?></td>
		  			<td><?= $row->code; ?></td>
		  			<td><?= $row->document_date; ?></td>
		  			<td><?= $row->vendor_code.' - '.ucfirst($row->vendor_name); ?></td>
...
<script>
	$(function() {
	    ...
	    $('#print').click(function(){
	        var urlNew = "<?= base_url($controller_id)?>/purchase/1?start=<?= $start;?>&end=<?= $end; ?>";
